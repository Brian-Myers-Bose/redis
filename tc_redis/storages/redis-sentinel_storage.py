...
class Storage(BaseStorage):
    ...
    def reconnect_redis(self):
        ...
        logger.info("Am sentinel: " + str(self.context.config.REDIS_IS_SENTINEL))
        if self.context.config.REDIS_IS_SENTINEL is True:
            logger.info("Sentinel path recognized")
            sentinel = Sentinel(self.context.config.REDIS_STORAGE_SERVER_SENTINELS, socket_timeout=0.1)
            logger.info("sentinel object: " + str(sentinel))
            master = sentinel.master_for('mymaster', socket_timeout=0.1)
            logger.info("I retrieved master")
            logger.info("Info: " + str(master.info))
            master.set('foo', 'bar')
            logger.info('Asked master for "foo", received: ' + str(master.get('foo')))
